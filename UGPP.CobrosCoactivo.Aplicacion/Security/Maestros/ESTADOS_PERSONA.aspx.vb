...
Partial Public Class ESTADOS_PERSONA
    ...
    Private Function GetSQL() As String
        Dim sql As String = ""
        sql = sql & "select [dbo].[ESTADOS_PERSONA].* from [dbo].[ESTADOS_PERSONA]"
        Dim WhereClause As String = ""
        If txtSearchcodigo.Text.Length > 0 Then
            WhereClause = WhereClause & " and [dbo].[ESTADOS_PERSONA].[codigo] like @codigo"

        End If

        If txtSearchnombre.Text.Length > 0 Then
            WhereClause = WhereClause & " and [dbo].[ESTADOS_PERSONA].[nombre] like @nombre"
        ...
        If Len(Session("ESTADOSUnderScorePERSONA.SortExpression")) > 0 Then
            sql = sql & " order by " & Session("ESTADOSUnderScorePERSONA.SortExpression") & " " & Session("ESTADOSUnderScorePERSONA.SortDirection")
    ...
    Private Overloads Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        Dim InsertSQL As String = "Insert into [dbo].[ESTADOS_PERSONA] ([codigo], [nombre] ) VALUES ( @codigo, @nombre ) "

        ' 
        'Declare String UpdateSQL 
        Dim UpdateSQL As String = "Update [dbo].[ESTADOS_PERSONA] set [codigo] = @codigo, [nombre] = @nombre where [codigo] = @codigo "
        ...
        Try
            ...
            LogProc.SaveLog(Session("ssloginusuario"), "Estados de personas", "Código " & ID, Command)
    ...
    Protected Sub cmdLoad_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdLoad.Click
        ...
        If Len(ID.Text) > 0 Then
            Dim sql As String = "select * from [dbo].[ESTADOS_PERSONA] where [codigo] = @codigo"
            ' 
            'Declare SQLCommand Object named Command
    ...
    Protected Sub grd_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles grd.Sorting
        ...
        Select Case CStr(Session("ESTADOSUnderScorePERSONA.SortDirection"))
            Case "ASC"
                Session("ESTADOSUnderScorePERSONA.SortDirection") = "DESC"
            Case "DESC"
                Session("ESTADOSUnderScorePERSONA.SortDirection") = "ASC"
            Case Else
                Session("ESTADOSUnderScorePERSONA.SortDirection") = "ASC"
        End Select

        Session("ESTADOSUnderScorePERSONA.SortExpression") = e.SortExpression
    ...
    Protected Sub cmdDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        If Len(ID.Text) > 0 Then
            ...
            Dim sql As String = "delete from [dbo].[ESTADOS_PERSONA] where [codigo] = @codigo"
            ' 
            'Declare SQLCommand Object named Command
            ...
            Try
                ...
                'Después de cada GRABAR hay que llamar al log de auditoria
                Dim LogProc As New LogProcesos
                LogProc.SaveLog(Session("ssloginusuario"), "Estados de personas", "Código " & ID.Text.Trim, Command)
