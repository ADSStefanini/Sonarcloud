...
Partial Public Class EditEJEFISGLOBALREPARTIDOR
        ...
        If IsPostBack = False Then
            ...
            If Len(Request("ID")) > 0 Then
                ...
                If Reader.Read Then
                    ...
                    txtEFIFECENTGES.Text = Format(Now, "short Date")
                    'Left(Reader("EFIFECENTGES").ToString().Trim, 10)
    ...
    Protected Sub LoadcboEFIUSUASIG()
        ...
        Dim cmd As String = "SELECT USUARIOS.codigo, USUARIOS.nombre " &
                             "FROM USUARIOS " &
                             "WHERE useractivo = 1 " &
                               "ORDER BY nivelacces "
    ...
    Protected Sub LoadcboRepartidor()
        Dim cnx As String = Funciones.CadenaConexion
        'Dim cmd As String = "SELECT codigo, nombre FROM usuarios ORDER BY nombre"
        Dim cmd As String = "SELECT DISTINCT USUARIOS.codigo, USUARIOS.login, USUARIOS.nombre, USUARIOS.nivelacces AS perfil, " &
    ...
    Private Overloads Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        'Comandos SQL
        Dim InsertSQL As String = "Insert into [EJEFISGLOBAL] ([EFINROEXP], [EFIFECHAEXP], [EFINUMMEMO], [EFIEXPORIGEN],  [EFIEXPDOCUMENTIC],[EFIMODCOD], [EFIESTADO] ) VALUES ( @EFINROEXP, @EFIFECHAEXP, @EFINUMMEMO, @EFIEXPORIGEN,@EFIEXPDOCUMENTIC, @EFIMODCOD, @EFIESTADO) "
        Dim UpdateSQL As String = "Update [EJEFISGLOBAL] set [EFIFECHAEXP] = @EFIFECHAEXP, [EFINUMMEMO] = @EFINUMMEMO, [EFIEXPORIGEN] = @EFIEXPORIGEN, [EFIEXPDOCUMENTIC] =@EFIEXPDOCUMENTIC,[EFIESTADO] = @EFIESTADO  WHERE [EFINROEXP] = @EFINROEXP "
        ' 
        ...
        ' Validar No. Expediente documentic 
        If txtEFIEXPDOCUMENTIC.Text.Trim = "" Then
            CustomValidator1.ErrorMessage = "Digite el No. Expediente de Documentic   por favor"
            CustomValidator1.IsValid = False
            Connection.Close()
            Return
        Else
            Command.Parameters.AddWithValue("@EFIEXPDOCUMENTIC", txtEFIEXPDOCUMENTIC.Text.Trim)
        End If
    ...
    Private Function GetEstadoInicialProceso() As Boolean
        ...
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "SELECT TOP 1 resultado FROM CLASIFICACION_CARTERA " &
    ...
    Private Sub GrabarClasificacioCartera(ByVal pExpediente As String)
        ...
        If ExisteClasificacionCartera(pExpediente) Then
            'update
            sql = "UPDATE CLASIF_CARTERA_REGISTRO SET " &
                    "TitEjecAntig = @TitEjecAntig, Caducidad = @Caducidad, EstadoPersona = @EstadoPersona, " &
                    "ProcesoCurso = @ProcesoCurso, AcuerdoPago = @AcuerdoPago, EstadoInicial = @EstadoInicial " &
                    "WHERE NroExp = @NroExp"
        Else
            'insert
            sql = "INSERT INTO CLASIF_CARTERA_REGISTRO (NroExp, TitEjecAntig, Caducidad, EstadoPersona, ProcesoCurso, AcuerdoPago, EstadoInicial) " &
    ...
    Protected Sub cmdSave2_Click(ByVal sender As Object, ByVal e As EventArgs) Handles cmdSave2.Click
        ...
        If HayCambioEstado(ID, cboRepartidor.SelectedValue, idRevisor, cboEFIUSUASIG.SelectedValue, cboEFIESTADO.SelectedValue, cboEFIESTADOPAGO.SelectedValue, HayCambioGestor) Then
            ...
            Dim FechaEntregaGestor As Date
            Try
                FechaEntregaGestor = CType(txtEFIFECENTGES.Text.Trim, Object)
            Catch
                FechaEntregaGestor = Format(Now, "short Date")
            End Try
    ...
    Private Sub RegistrarFecEstiFin(ByVal pExpediente As String)
        ...
        If swExisteRegistro Then
            ...
            ' sql = "UPDATE persuasivo SET FecEstiFin = (select DATEADD(month, 1, max(efifecentges))  from EJEFISGLOBAL where EFINROEXP ='" & pExpediente & "') WHERE NroExp = '" & pExpediente & "'"
            'End If
        Else
            'Insertar registro en tabla de persuasivo
            sql = "INSERT INTO persuasivo (NroExp, FecEstiFin) VALUES ('" & pExpediente & "', (select DATEADD(month, 5, max(efifecentges))  from EJEFISGLOBAL where EFINROEXP ='" & pExpediente & "') )"
    ...
    Private Function GetSQL() As String
        ...
        sql = sql & "SELECT * FROM SOLICITUDCAMBIOESTADO WHERE NroExp = '" & Request("ID") & "' order by fecha desc"
