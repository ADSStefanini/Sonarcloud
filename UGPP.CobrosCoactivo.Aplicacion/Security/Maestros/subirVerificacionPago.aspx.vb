...
Namespace Security.Maestros
    ...
    Partial Public Class SubirVerificacionPago
        ...
        Private Sub ActualizarTablaPagos(ByVal pExpediente As String, ByVal connection As SqlConnection, ByVal tran As SqlTransaction, ByVal LogProc As LogProcesos)
            Dim sql As String
            sql = "SELECT NroConsignacion, MAX(Expediente) AS Expediente, MAX(FecRepPagGR) AS FecRepPagGR, " &
                  "	MAX(FecVerifPago) AS FecVerifPago, MAX(FecRepPagDeudor) AS FecRepPagDeudor," &
                  "	SUM(CapitalPag) AS CapitalPag, " &
                  "	SUM(AjusteDec1406) AS AjusteDec1406, SUM(InteresPag) AS InteresPag," &
                  "	SUM(gastosproc) AS gastosproc, SUM(PagosExceso) AS PagosExceso," &
                  "	SUM(TotalPag) AS TotalPag," &
                  "	MAX(EstadoPag) AS EstadoPag," &
                  "	MAX(EstadoProcFRP) AS EstadoProcFRP " &
                  "FROM pagos_detalle " &
        ...
        Private Sub GuardarPago(ByVal nroConsignacion As String, ByVal expediente As String, ByVal fecRepPagGr As Date, ByVal fecVerifPago As Date, ByVal fecRepPagDeudor As Date, ByVal capitalPag As Double, ByVal ajusteDec1406 As Double, ByVal interesPag As Double, ByVal gastosproc As Double, ByVal pagosExceso As Double, ByVal totalPag As Double, ByVal estadoPag As String, ByVal estadoProcFrp As String, ByVal connection As SqlConnection, ByVal trans As SqlTransaction, ByVal LogProc As LogProcesos)
            ...
            Const insertSql As String = "INSERT INTO pagos (NroConsignacion, NroExp, FecSolverif, FecVerificado, pagFechaDeudor, pagCapital, pagAjusteDec1406, pagInteres, pagGastosProc, pagExceso, pagTotal, estado, pagestadoprocfrp, UserVerif, pagFecha) " &
                                        "VALUES " &
            ...
            Const updateSql As String = "UPDATE PAGOS SET " &
                                        "		NroConsignacion = @NroConsignacion, NroExp = @NroExp, FecSolverif = @FecSolverif, FecVerificado = @FecVerificado, " &
                                        "		pagFechaDeudor = @pagFechaDeudor, pagCapital = @pagCapital, pagAjusteDec1406 = @pagAjusteDec1406, pagInteres = @pagInteres, " &
                                        "		pagGastosProc = @pagGastosProc, pagExceso = @pagExceso, pagTotal = @pagTotal, estado = @estado, pagestadoprocfrp = @pagestadoprocfrp, " &
                                        "		UserVerif = @UserVerif, pagFecha = @pagFecha " &
