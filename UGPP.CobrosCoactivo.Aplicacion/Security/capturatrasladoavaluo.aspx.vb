    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.Page.IsPostBack Then
            ...
            If sihaydoc = True Then
                ...
                txtperito.Text = myTable.Rows(0).Item("doc_nombresecuestre")
                txtavaluo.Text = myTable.Rows(0).Item("doc_liqtot")
                txtfecha.Text = CDate(myTable.Rows(0).Item("doc_fechadiligenciasecuestre"))
                txthonorarioperito.Text = myTable.Rows(0).Item("doc_liqint")
    ...
    Protected Sub btnImprimir_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnImprimir.Click
        ...
        Using connection As New SqlConnection(IIf(Session("ConexionServer").ToString = Nothing, Funciones.CadenaConexion, Session("ConexionServer")))
            ...
            Try
                ...
                command.Parameters.AddWithValue("@nombresecuestre", txtperito.Text)
                command.Parameters.AddWithValue("@doc_liqtot", txtavaluo.Text)
                command.Parameters.AddWithValue("@doc_liqint", txthonorarioperito.Text)
                command.Parameters.AddWithValue("@doc_fechadiligenciasecuestre", CDate(txtfecha.Text))



                If actualizar = 1 Then
                    command.CommandText = UCase("UPDATE entra_documentoma SET doc_liqint=@doc_liqint, doc_fechadiligenciasecuestre=@doc_fechadiligenciasecuestre, doc_deudornombre=@doc_deudornombre, doc_deudorcedula=@doc_deudorcedula, doc_nombresecuestre=@nombresecuestre, doc_liqtot=@doc_liqtot, doc_expediente=@EXPEDIENTE,doc_actoadministrativo=@actoadmin,doc_impuesto=@IMPUESTO WHERE doc_expediente =@EXPEDIENTE and doc_actoadministrativo = @actoadmin")

                Else
                    command.CommandText = UCase("INSERT INTO entra_documentoma (doc_liqint,doc_fechadiligenciasecuestre,doc_deudorcedula,doc_deudornombre,doc_expediente,doc_actoadministrativo,doc_impuesto,doc_nombresecuestre,doc_liqtot) VALUES(@doc_liqint,@doc_fechadiligenciasecuestre,@doc_deudorcedula,@doc_deudornombre,@EXPEDIENTE,@actoadmin,@IMPUESTO, @nombresecuestre,@doc_liqtot)")
                End If
