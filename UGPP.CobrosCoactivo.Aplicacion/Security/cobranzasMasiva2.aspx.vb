...
Partial Public Class cobranzasMasiva2
    ...
    Public Function cargar_representante(ByVal expediente As Integer, Optional ByVal OP As Integer = 1) As String
        ...
        Dim sqldata As New SqlDataAdapter(" SELECT A.ED_NOMBRE ,A.ED_CODIGO_NIT" &
                                          " FROM  ENTES_DEUDORES A, " &
                                          " DEUDORES_EXPEDIENTES B, " &
                                          " EJEFISGLOBAL C " &
                                          " WHERE  C.EFINROEXP =B.NROEXP  " &
                                          " AND A.ED_CODIGO_NIT = B.DEUDOR " &
            ...
            For i As Integer = 0 To exp.Length
                ...
                Select Case CodRep
                    ...
                    Case 217, 219, 221, 222, 301, 307, 329
                        sql =
                        "SELECT A.EFINROEXP AS MAN_EXPEDIENTE,																																																		" &
                        ...
                        " 	   E.MT_nro_titulo,F.nombre AS MT_tipo_titulo, E.MT_fec_expedicion_titulo,E.MT_fec_notificacion_titulo,'$ '+CAST(CONVERT(varchar, CAST(E.totaldeuda AS money), 1) AS varchar) as Total_Deuda,E.totaldeuda,                                                                           " &
                        ...
                        " 	   k.nombre as Proyecto,                                                                                                                                                                                                                " &
                        " 	   L.nombre as Revisor,                                                                                                                                                                                                                 " &
                        " 	   D.TIPO as Tipo_Deudor,                                                                                                                                                                                                               " &
                        " 	   c.idunico as IdDireccion,F.codigo as codigotitulo,A.EFINUMMEMO" & IIf(ColumnasAdicionales = "", "", "," & ColumnasAdicionales & " ") &
                        "FROM  EJEFISGLOBAL A,ENTES_DEUDORES B, DIRECCIONES C, DEUDORES_EXPEDIENTES D,MAESTRO_TITULOS E,TIPOS_TITULO F,TIPOS_IDENTIFICACION G,                                                                                                                   " &
                        "		DEPARTAMENTOS H,MUNICIPIOS I, USUARIOS K,  USUARIOS L                                                                                                                                                                     " &
                        "WHERE A.EFINROEXP = D.NROEXP AND D.DEUDOR = B.ED_CODIGO_NIT AND B.ED_CODIGO_NIT = C.DEUDOR AND E.MT_EXPEDIENTE = A.EFINROEXP                                                                                                                            " &
                        "		AND   F.CODIGO = E.MT_TIPO_TITULO  AND   G.CODIGO  = B.ED_TIPOID AND   H.CODIGO = C.DEPARTAMENTO AND  I.CODIGO = C.CIUDAD                                                                                                                        " &
                        "	    AND K.CODIGO = A.EFIUSUASIG AND L.CODIGO = A.EFIUSUREV                                                                                                                                             " &
                        " " & parametros & " " &
                        " " & exp(i) & "                                                                                                                                                                                                                 " &
                    ...
                    Case 228, 13, 233, 354, 355, 356, 226, 363
                        ...
                        " 	   G.nombre AS ED_TipoId,(ED_CODIGO_NIT + CASE WHEN ISNULL(ED_DigitoVerificacion,'') ='' THEN '' ELSE '-' END +CASE WHEN ED_DIGITOVERIFICACION =''THEN '' ELSE ISNULL(ED_DIGITOVERIFICACION,'') END) AS ED_Codigo_Nit, B.ED_Nombre, CASE WHEN ED_TipoPersona = '02' THEN 'Señor(a)' ELSE 'Doctor(a)' END AS ED_TipoPersona, CASE WHEN ED_TipoPersona = '02' THEN 'Cuidadano' ELSE 'Empresario(a)' END AS Cuidadano,  " &
                        " 	   C.Direccion,CASE WHEN C.EMAIL ='SIN DATOS' THEN '' ELSE C.EMAIL END AS EMAIL, (case when isnull( C.Movil,'') = '' then '' else  case when isnull( C.Movil,'')='Sin datos'or isnull( C.Movil ,'')= 'sin datos'or isnull( C.Movil ,'')= 'sin dato' then '' else C.Movil end end)	 + 		(case when isnull( C.Movil,'')='Sin datos'or isnull( C.Movil ,'')= 'sin datos'or isnull( C.Movil ,'')= 'sin dato' or isnull( C.Telefono ,'')='Sin datos' or isnull( C.Telefono ,'')= 'sin datos'or isnull( C.Telefono ,'')= 'sin dato' then '' else ' - ' end)		 + 	    (case when isnull( C.Telefono ,'') = '' then '' else case when isnull( C.Telefono ,'')='Sin datos' or isnull( C.Telefono ,'')= 'sin datos'or isnull( C.Telefono ,'')= 'sin dato' then '' else C.Telefono  end end ) as telefono,                                                                                     " &
                        " 	   E.MT_nro_titulo,F.nombre AS MT_Tipo_Titulo, E.MT_fec_expedicion_titulo,E.MT_fec_notificacion_titulo,J.nombre AS MT_for_notificacion_titulo  ,'$ '+CAST(CONVERT(varchar, CAST(E.totaldeuda AS money), 1) AS varchar) as Total_Deuda,E.totaldeuda,                                                                           " &
                        " 	   CASE WHEN H.NOMBRE ='SIN DATOS'THEN '' ELSE H.NOMBRE END AS Departamento,                                                                                                                                                                                                            " &
                        " 	   CASE WHEN I.NOMBRE ='SIN DATOS'THEN '' ELSE I.NOMBRE END AS Municipio,                                                                                                                                                                                                                 " &
                        " 	   k.nombre as Proyecto,                                                                                                                                                                                                                " &
                        " 	   L.nombre as Revisor,                                                                                                                                                                                                                 " &
                        " 	   D.TIPO as Tipo_Deudor,                                                                                                                                                                                                               " &
                        " 	   c.idunico as IdDireccion,A.EFINUMMEMO,E.MT_fecha_ejecutoria,F.codigo as codigotitulo,A.EFIFECHAEXP" & IIf(ColumnasAdicionales = "", "", "," & ColumnasAdicionales & " ") & "" &
                        " FROM  EJEFISGLOBAL A,                                                                                                                                                                                                                     " &
                        " 	  ENTES_DEUDORES B,                                                                                                                                                                                                                     " &
                        " 	  DIRECCIONES C ,                                                                                                                                                                                                                       " &
                    ...
                    Case Else
                        ...
                        " 	   G.nombre AS ED_TipoId,ED_CODIGO_NIT + CASE WHEN ISNULL(ED_DigitoVerificacion,'') ='' THEN '' ELSE '-' END +CASE WHEN ED_DIGITOVERIFICACION =''THEN '' ELSE ISNULL(ED_DIGITOVERIFICACION,'') END AS ED_Codigo_Nit, B.ED_Nombre, CASE WHEN ED_TipoPersona = '02' THEN 'Señor(a)' ELSE 'Doctor(a)' END AS ED_TipoPersona, CASE WHEN ED_TipoPersona = '02' THEN 'Cuidadano' ELSE 'Empresario(a)' END AS Cuidadano,  " &
                        " 	   C.Direccion,CASE WHEN C.EMAIL ='SIN DATOS' THEN '' ELSE C.EMAIL END AS Email, (case when isnull( C.Movil,'') = '' then '' else  case when isnull( C.Movil,'')='Sin datos'or isnull( C.Movil ,'')= 'sin datos'or isnull( C.Movil ,'')= 'sin dato' then '' else C.Movil end end)	 + 		(case when isnull( C.Movil,'')='Sin datos'or isnull( C.Movil ,'')= 'sin datos'or isnull( C.Movil ,'')= 'sin dato' or isnull( C.Telefono ,'')='Sin datos' or isnull( C.Telefono ,'')= 'sin datos'or isnull( C.Telefono ,'')= 'sin dato' then '' else ' - ' end)		 + 	    (case when isnull( C.Telefono ,'') = '' then '' else case when isnull( C.Telefono ,'')='Sin datos' or isnull( C.Telefono ,'')= 'sin datos'or isnull( C.Telefono ,'')= 'sin dato' then '' else C.Telefono  end end ) as Telefono,                                                                                     " &
                        " 	   E.MT_nro_titulo,F.nombre AS MT_tipo_titulo, E.MT_fec_expedicion_titulo,isnull(MT_fec_notificacion_titulo ,00) as MT_fec_notificacion_titulo,J.nombre AS MT_for_notificacion_titulo  ,'$ '+CAST(CONVERT(varchar, CAST(E.totaldeuda AS money), 1) AS varchar) as Total_Deuda,E.totaldeuda,                                                                           " &
                        " 	   CASE WHEN H.NOMBRE ='SIN DATOS'THEN '' ELSE H.NOMBRE END AS Departamento,                                                                                                                                                                                                            " &
                        " 	   CASE WHEN I.NOMBRE ='SIN DATOS'THEN '' ELSE I.NOMBRE END AS Municipio,                                                                                                                                                                                                                 " &
                        " 	   k.nombre as Proyecto,                                                                                                                                                                                                                " &
                        " 	   L.nombre as Revisor,                                                                                                                                                                                                                 " &
                        ...
                        " 	   c.idunico as IdDireccion,F.codigo as codigotitulo,A.EFINUMMEMO" & IIf(ColumnasAdicionales = "", "", "," & ColumnasAdicionales & " ") &
                        "FROM  EJEFISGLOBAL A,ENTES_DEUDORES B, DIRECCIONES C, DEUDORES_EXPEDIENTES D,MAESTRO_TITULOS E,TIPOS_TITULO F,TIPOS_IDENTIFICACION G,                                                                                                                   " &
                        "		DEPARTAMENTOS H,MUNICIPIOS I, USUARIOS K,  USUARIOS L,FORMAS_NOTIFICACION J                                                                                                                                                                      " &
                        "WHERE A.EFINROEXP = D.NROEXP AND D.DEUDOR = B.ED_CODIGO_NIT AND B.ED_CODIGO_NIT = C.DEUDOR AND E.MT_EXPEDIENTE = A.EFINROEXP                                                                                                                            " &
                        "		AND   F.CODIGO = E.MT_TIPO_TITULO  AND   G.CODIGO  = B.ED_TIPOID AND   H.CODIGO = C.DEPARTAMENTO AND  I.CODIGO = C.CIUDAD                                                                                                                        " &
                        "		AND   J.CODIGO = isnull(MT_for_notificacion_titulo ,00) AND K.CODIGO = A.EFIUSUASIG AND L.CODIGO = A.EFIUSUREV                                                                                                                                             " &
                        " " & parametros & " " &
                        "AND A.EFINROEXP ='" & exp(i) & " '                                                                                                                                                                                                                " &
                        "ORDER BY A.EFINROEXP"
    ...
    Private Sub Imprimir(ByVal CodInforme As String)
        ...
        Select Case CodInforme
            ...
            Case "319-1"
                ...
                Dim ant As String
                If LLenarDatos(TbDatos, 319, " '' as fecha1       ,  '' as letras,            '' as Deuda_Valor , '' as nro_anterior ,  '' as fecha_resolucion," &
            ...
            Case "320-1"
                ...
                Dim embargos As Integer = Limite.Text.Trim
                Dim ant As String
                If LLenarDatos(TbDatos, 244, " '' as fecha1       ,  '' as letras,            '' as Deuda_Valor , '' as nro_anterior ,  '' as fecha_resolucion," &
