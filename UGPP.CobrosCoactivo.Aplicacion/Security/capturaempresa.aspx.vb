...
Partial Public Class capturaempresa
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not Me.Page.IsPostBack Then
            ...
            If sihaydoc = True Then
                ...
                txtEmpresa.Text = myTable.Rows(0).Item("doc_nombresecuestre".ToString)
                txtcertificacion.Text = myTable.Rows(0).Item("doc_detalle1".ToString)
                txtde.Text = myTable.Rows(0).Item("doc_fechadiligenciasecuestre".ToString)
    ...
    Protected Sub btnImprimir_Click(ByVal sender As Object, ByVal e As EventArgs) Handles btnImprimir.Click
        ...
        Using connection As New SqlConnection(IIf(Session("ConexionServer").ToString = Nothing, Funciones.CadenaConexion, Session("ConexionServer")))
            ...
            Try
                ...
                command.Parameters.AddWithValue("@doc_deudirest", tabla.Rows(0).Item(4).ToString)
                command.Parameters.AddWithValue("@doc_refcatrastal", tabla.Rows(0).Item(1).ToString)
                command.Parameters.AddWithValue("@doc_liqtot", tabla.Rows(0).Item(7))


                command.Parameters.AddWithValue("@nombresecuestre", txtEmpresa.Text)
                command.Parameters.AddWithValue("@doc_detalle1", txtcertificacion.Text)
                command.Parameters.AddWithValue("@doc_fechadiligenciasecuestre", txtde.Text)


                If actualizar = 1 Then
                    command.CommandText = UCase("UPDATE entra_documentoma SET  doc_liqtot=@doc_liqtot, doc_refcatrastal=@doc_refcatrastal, doc_deudirest=@doc_deudirest, doc_fechadiligenciasecuestre=@doc_fechadiligenciasecuestre, doc_detalle1=@doc_detalle1, doc_deudornombre=@doc_deudornombre, doc_deudorcedula=@doc_deudorcedula, doc_nombresecuestre=@nombresecuestre,  doc_expediente=@EXPEDIENTE,doc_actoadministrativo=@actoadmin,doc_impuesto=@IMPUESTO WHERE doc_expediente =@EXPEDIENTE and doc_actoadministrativo = @actoadmin")

                Else
                    command.CommandText = UCase("INSERT INTO entra_documentoma (doc_liqtot, doc_refcatrastal,doc_deudirest,doc_detalle1,doc_fechadiligenciasecuestre, doc_deudorcedula,doc_deudornombre,doc_expediente,doc_actoadministrativo,doc_impuesto,doc_nombresecuestre) VALUES(@doc_liqtot,@doc_refcatrastal,@doc_deudirest,@doc_detalle1,@doc_fechadiligenciasecuestre,@doc_deudorcedula,@doc_deudornombre,@EXPEDIENTE,@actoadmin,@IMPUESTO, @nombresecuestre)")
                End If
