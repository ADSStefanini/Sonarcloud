...
Partial Public Class TIPOS_IDENTIFICACION
    ...
    Private Function GetSQL() As String
        Dim sql As String = ""
        sql = sql & "select [dbo].[TIPOS_IDENTIFICACION].* from [dbo].[TIPOS_IDENTIFICACION]"
        Dim WhereClause As String = ""
        If txtSearchcodigo.Text.Length > 0 Then
            WhereClause = WhereClause & " and [dbo].[TIPOS_IDENTIFICACION].[codigo] like @codigo"

        End If

        If txtSearchnombre.Text.Length > 0 Then
            WhereClause = WhereClause & " and [dbo].[TIPOS_IDENTIFICACION].[nombre] like @nombre"
        ...
        If Len(Session("TIPOSUnderScoreIDENTIFICACION.SortExpression")) > 0 Then
            sql = sql & " order by " & Session("TIPOSUnderScoreIDENTIFICACION.SortExpression") & " " & Session("TIPOSUnderScoreIDENTIFICACION.SortDirection")
    ...
    Private Overloads Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        Dim InsertSQL As String = "Insert into [dbo].[TIPOS_IDENTIFICACION] ([codigo], [nombre] ) VALUES ( @codigo, @nombre ) "

        ' 
        'Declare String UpdateSQL 
        Dim UpdateSQL As String = "Update [dbo].[TIPOS_IDENTIFICACION] set [codigo] = @codigo, [nombre] = @nombre where [codigo] = @codigo "
        ...
        Try
            ...
            LogProc.SaveLog(Session("ssloginusuario"), "Tipos de identificación", "Código " & ID.Trim, Command)
    ...
    Protected Sub cmdLoad_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdLoad.Click
        ...
        If Len(ID.Text) > 0 Then
            Dim sql As String = "select * from [dbo].[TIPOS_IDENTIFICACION] where [codigo] = @codigo"
            ' 
            'Declare SQLCommand Object named Command
    ...
    Protected Sub grd_Sorting(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.GridViewSortEventArgs) Handles grd.Sorting
        ...
        Select Case CStr(Session("TIPOSUnderScoreIDENTIFICACION.SortDirection"))
            Case "ASC"
                Session("TIPOSUnderScoreIDENTIFICACION.SortDirection") = "DESC"
            Case "DESC"
                Session("TIPOSUnderScoreIDENTIFICACION.SortDirection") = "ASC"
            Case Else
                Session("TIPOSUnderScoreIDENTIFICACION.SortDirection") = "ASC"
        End Select

        Session("TIPOSUnderScoreIDENTIFICACION.SortExpression") = e.SortExpression
    ...
    Protected Sub cmdDelete_Click(ByVal sender As Object, ByVal e As System.EventArgs) Handles cmdDelete.Click
        If Len(ID.Text) > 0 Then
            ...
            Dim sql As String = "delete from [dbo].[TIPOS_IDENTIFICACION] where [codigo] = @codigo"
            ' 
            'Declare SQLCommand Object named Command
            ...
            Try
                ...
                'Después de cada GRABAR hay que llamar al log de auditoria
                Dim LogProc As New LogProcesos
                LogProc.SaveLog(Session("ssloginusuario"), "Tipos de identificación", "Código " & ID.Text.Trim, Command)
