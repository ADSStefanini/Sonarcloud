...
Public Class MetodosGlobalesCobro
    ...
    Public Sub AlertarEscalar(ByVal pNumAlarma As Integer, ByVal pNumdias As Integer, ByVal pUsuarioOrigen As String, ByVal pNroExp As String, ByRef pTermino As String, ByRef pExplicacion As String, ByRef pPictureURL As String, ByVal dtAlarmas As DataTable, ByVal pComparador1 As String, ByVal pComparador2 As String)
    ...
    Public Sub AjustarPagos(ByRef dtProcs As DataTable)
        ...
        For x = 0 To dtProcs.Rows.Count - 1
            ...
            dtProcs.Rows(x).Item("EFISALDOCAP") = Convert.ToDouble(dtProcs.Rows(x).Item("EFIVALDEU").ToString()) - PagoyAjuste
        Next
    ...
    Public Sub AjustarTerminos(ByRef dtProcs As DataTable, ByVal emisor As String)
        ...
        For x = 0 To dtProcs.Rows.Count - 1
            ...
            If HaySolCambioE Then
                ...
                AlertarEscalar(NumAlarma, DiasHabiles, pUsuarioOrigen, dtProcs.Rows(x).Item("EFINROEXP").ToString(), dtProcs.Rows(x).Item("termino"), dtProcs.Rows(x).Item("explicacion"), dtProcs.Rows(x).Item("PictureURL"), dtAlarmas, "=", ">=")
            ...
            ElseIf NombreEstado = "COACTIVO" Then
                ...
                Else
                    ...
                    If FechaNotifMP = Nothing Then
                        ...
                        dtProcs.Rows(x).Item("termino") = "AE9."
                ...
                If FechaRadicExcepciones <> Nothing Then
                    ...
                    If NroResResuelve = "" Then
                        ...
                        AlertarEscalar(NumAlarma, DiasCalendario, pUsuarioOrigen, dtProcs.Rows(x).Item("EFINROEXP").ToString(), dtProcs.Rows(x).Item("termino"), dtProcs.Rows(x).Item("explicacion"), dtProcs.Rows(x).Item("PictureURL"), dtAlarmas, ">=", ">=")
    ...
    Public Function ExisteResFac(ByVal pExpediente As String) As Boolean
        ...
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "SELECT TOP 1 DG_ID FROM DOCUMENTOS_GENERADOS " & _
                                "	WHERE DG_EXPEDIENTE = '" & pExpediente & "' AND " & _
    ...
    Public Function ExisteOficioPresentacion(ByVal pExpediente As String) As Boolean
        ...
        Dim Reader As SqlDataReader = Command.ExecuteReader
        If Reader.Read Then
            If Reader("NroOfiPres").ToString().Trim <> "" And _
                Reader("NroOfiPres").ToString().Trim <> "sin datos" And _
                Reader("NroOfiPres").ToString().Trim <> "NA" And _
    ...
    Public Function GetFechaPagoRec(ByVal pExpediente As String, ByVal pFecCuota As String) As Date
        ...
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "SELECT FECHA_PAGO " & _
                            "	FROM DETALLES_ACUERDO_PAGO " & _
                            "	WHERE DOCUMENTO = (SELECT DOCUMENTO FROM MAESTRO_ACUERDOS WHERE expediente = '" & pExpediente & "' AND ESTADO = 'I') AND " & _
    ...
    Public Function GetFechaUltimaCuota(ByVal pExpediente As String) As Date
        ...
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "SELECT MAX(FECHA_CUOTA) AS UltimaCuota " & _
                            "	FROM DETALLES_ACUERDO_PAGO " & _
                            "	WHERE DOCUMENTO = (SELECT DOCUMENTO FROM MAESTRO_ACUERDOS " & _
    ...
    Public Function GetFechaCuotaReciente(ByVal pExpediente As String) As Date
        ...
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "SELECT MAX(FECHA_CUOTA) AS FecCuoRecent " & _
                            "	FROM DETALLES_ACUERDO_PAGO " & _
                            "	WHERE DOCUMENTO = (SELECT DOCUMENTO FROM MAESTRO_ACUERDOS WHERE expediente = '" & pExpediente & "' AND ESTADO = 'I') AND " & _
    ...
    Public Sub RegistrarMensaje(ByVal pNroExp As String, ByVal pUsuOrigen As String, ByVal pUsuDestino As String, ByVal pMensaje As String, ByVal pFecEnvio As DateTime, Optional ByVal MsgSistema As Boolean = False)
        ...
        'Comandos SQL
        Dim InsertSQL As String = "INSERT INTO mensajes (NroExp, UsuOrigen, UsuDestino, Mensaje, FecEnvio) " & _
    ...
    Public Function RegistrarUsuario(ByVal pIdUsuario As String, ByVal pNivel As Integer, Optional ByVal pSuperior As String = "") As String
        ...
        Dim Command As SqlCommand
        'Comandos SQL
        Dim InsertSQL As String = "INSERT INTO usuarios (codigo, nombre, documento, clave, nivelacces, cobrador, apppredial, appvehic, appcuotasp, appindycom, login, useractivo, useremail, usercamclave, superior) " & _
