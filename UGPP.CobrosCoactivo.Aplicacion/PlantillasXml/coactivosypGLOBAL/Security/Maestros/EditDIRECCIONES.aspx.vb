...
Partial Public Class EditDIRECCIONES
    ...
    Protected Sub LoadcboCiudad()

        'Create a new connection to the database
        Dim Connection As New SqlConnection(Funciones.CadenaConexion)
        Connection.Open()
        Dim sql As String = "select codigo, nombre from [MUNICIPIOS] order by nombre"
        Dim Command As New SqlCommand(sql, Connection)
    ...
    Private Overloads Sub cmdSave_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdSave.Click
        ...
        Response.Redirect("DIRECCIONES.aspx?ID=" & Request("ID") & "&pIdUnico=" & Request("pIdUnico=") & "&pExpediente=" & Request("pExpediente"))
